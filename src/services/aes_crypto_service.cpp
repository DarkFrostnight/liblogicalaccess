...
namespace logicalaccess {
    ...
    ByteVector AESCryptoService::aes_encrypt(const ByteVector &data, const ByteVector &iv, std::shared_ptr<Key> key) {
        perform_operation(data, iv, key, true);
    }

    ByteVector AESCryptoService::in_memory(const ByteVector &data, const ByteVector &iv, std::shared_ptr<Key> key,
                                           bool encrypt) {
    ...
    ByteVector AESCryptoService::with_pkcs(const ByteVector &data, const ByteVector &iv, std::shared_ptr<Key> key,
    bool encrypt) {
        std::shared_ptr<PKCSKeyStorage> storage = std::dynamic_pointer_cast<PKCSKeyStorage>(key->getKeyStorage());
    ...
    ByteVector
    AESCryptoService::perform_operation(const ByteVector &data, const ByteVector &iv, std::shared_ptr<Key> key,
                                        bool encrypt) {
        switch (key->getKeyStorage()->getType()) {
            case KST_COMPUTER_MEMORY:
                return in_memory(data, iv, key, encrypt);

            case KST_PKCS:
                return with_pkcs(data, iv, key, encrypt);
    ...
    ByteVector AESCryptoService::aes_decrypt(const ByteVector &data, const ByteVector &iv, std::shared_ptr<Key> key) {
