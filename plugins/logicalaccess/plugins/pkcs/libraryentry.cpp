...
namespace logicalaccess {
    ...
    public:
        ByteVector
        aes_encrypt(const ByteVector &data, const ByteVector &iv, std::shared_ptr<logicalaccess::Key> key) override {
            ...
            std::string pw_copy = storage->pkcs_session_password();
        ...
        ByteVector
        aes_decrypt(const ByteVector &data, const ByteVector &iv, std::shared_ptr<logicalaccess::Key> key) override {
            std::shared_ptr<PKCSKeyStorage> storage = std::dynamic_pointer_cast<PKCSKeyStorage>(key->getKeyStorage());
            EXCEPTION_ASSERT_WITH_LOG(storage, LibLogicalAccessException, "No key storage.");
    ...
    private:
        cppkcs::Object find_key(cppkcs::Session &session, const PKCSKeyStorage&storage) {
            ...
            return std::move(os.find_objects(cppkcs::make_attribute<CKA_ID>(storage.get_key_id())).at(0));
        }
    };
}
